<table class="table table-hover" id="example"  width="100%"> 
	<thead>
		<tr>
			...
			<th>Proyek</th>
			<th></th>
			<th>Nilai Hibah (Juta)</th>
			<th>Indikasi Donor</th>
	...
	<tbody>
		...
		<tr class="gradeX">
			...
			<td><a href="#" onclick="detail_list('hibah/usulan_detail/<?php echo $row->id; ?>');"><?php echo $row->judul_id; ?></a></td>
			<td><?php echo $row->mata_uang_singkatan; ?></td>
			<td><?php echo number_format(($row->nilai_hibah/1000000),2); ?></td>
			<td><?php echo $row->donor; ?></td>
			<td class="text-center">
				<?php 
					if($row->nilai_admin == 0){
						echo "<a href='#' type='button' class='btn btn-xs btn-danger' onclick='nilai_admin($row->id)'><i class='fa fa-times-circle'></i></a>";
					}else{
			...
			<td class="text-center">
				<?php 
					if($row->nilai_layak == FALSE){
						echo "<a href='#' type='button' class='btn btn-xs btn-danger' onclick='nilai_layak($row->id, $row->nilai_admin)'><i class='fa fa-times-circle'></i></a>";
					}else{
						echo "<span type='button' class='btn btn-xs btn-primary'><i class='fa fa-check-circle'></i></span>";
					}
			...
			<td class="text-center">
				<?php 
					if($row->masuk_drkh == FALSE){
						echo "<a href='#' type='button' class='btn btn-xs btn-danger' onclick='masuk_drkh($row->id, $row->nilai_admin, $row->nilai_layak)'><i class='fa fa-times-circle'></i></a>";
					}else{
						echo "<span type='button' class='btn btn-xs btn-primary'><i class='fa fa-check-circle'></i></span>";
					}
					//echo $row->masuk_drkh; 
			...
			<td>
				<div class="input-group-btn">
					...
					<ul class="dropdown-menu pull-right">
						...
						?>
						<li><a href="#" onclick="tampilkan_form_edit('hibah/usulan_tampil_form_edit/<?php echo $row->id; ?>')">Edit</a></li>
						<li class="divider"></li>
						<?php } ?>
						<li><a href="#" onclick="tampilkan_log_usulan_index('hibah/tampilkan_log_usulan_index/<?php echo $row->id; ?>')">Rekam Proyek</a></li>
						<li><a href="#" onclick="tampilkan_syarat_usulan('hibah/tampilkan_dok_usulan_index/<?php echo $row->id; ?>')">Dok. Persyaratan</a></li>
						<?php 
							if($this->session->userdata('id_user_level') == '3' || $this->session->userdata('id_user_level') == '1'){ 
						?>
						<li><a href="#" onclick="tampilkan_form_hapus('hibah/usulan_tampil_form_hapus/<?php echo $row->id; ?>')"><span class="text-danger" ><strong>Hapus</strong></span></a></li>
						<?php } ?>
					</ul>
				</div>
			</td>
...
<script>
	...
	function nilai_admin(id){
		$.ajax({
			url: "<?php echo base_url(); ?>hibah/usulan_tampil_form_nilai_admin/"+id,
			beforeSend: function(){
				showLoading();
			},
			success:function(response){
	...
	function nilai_layak(id, nilai_admin){
		...
		if(nilai_admin == '1'){
			$.ajax({
				url: "<?php echo base_url(); ?>hibah/usulan_tampil_form_nilai_layak/"+id,
				beforeSend: function(){
					showLoading();
				},
				success:function(response){
	...
	function masuk_drkh(id, nilai_admin, nilai_layak){
		if(nilai_admin == '1' && nilai_layak == '1'){
			$.ajax({
				url: "<?php echo base_url(); ?>hibah/usulan_tampil_form_masuk_drkh/"+id,
				beforeSend: function(){
					showLoading();
				},
				success:function(response){
